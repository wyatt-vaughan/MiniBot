...
class AStarOptimized(PathPlanner):
    ...
    def plan_movements(self, pieces: Dict[str, Piece], 
        ...
        for iteration in range(max_iterations):
            ...
            for piece_id in priorities:
                ...
                # Smooth the path to remove micro zigzags
                if path and len(path) > 2:
                    path = self._smooth_path(path, piece_id, simulated_positions)
        ...
        # Optimize move timing for parallelization
        print(f"\n=== Optimizing Move Timing ===")
        plan = self._optimize_parallel_execution(plan, simulated_positions)
    ...
    def _smooth_path(self, path: List[Tuple[float, float]], piece_id: str,
                    simulated_positions: Dict[str, Position]) -> List[Tuple[float, float]]:
        ...
        while current_idx < len(path) - 1:
            ...
            for test_idx in range(current_idx + 2, len(path)):
                if self._has_line_of_sight(path[current_idx], path[test_idx], piece_id, simulated_positions):
    ...
    def _has_line_of_sight(self, start: Tuple[float, float], end: Tuple[float, float],
                           piece_id: str, simulated_positions: Dict[str, Position]) -> bool:
        ...
        num_samples = max(int(distance / 5), 2)  # Sample every 5mm
        ...
        for i in range(num_samples + 1):
            ...
            if not self._is_grid_position_valid(test_x, test_y, piece_id, simulated_positions):
                return False
        
        return True
    
    def _optimize_parallel_execution(self, plan: ExecutionPlan,
                                     initial_positions: Dict[str, Position]) -> ExecutionPlan:
        """Optimize execution timing to allow safe parallel moves"""
        # Get all sequences with their original times and piece IDs
        all_sequences = []
        for piece_id, sequences in plan.sequences.items():
            for seq in sequences:
                all_sequences.append((piece_id, seq, seq.start_time))
        ...
        # Track when each piece finishes its current movement and where it is
        piece_finish_times = {}
        piece_positions_after_moves = {pid: pos.copy() for pid, pos in initial_positions.items()}
        
        # Process sequences in order, trying to start them as early as possible
        parallelized_count = 0
        
        for piece_id, sequence, original_start_time in all_sequences:
            # Earliest this piece can start (after its previous move)
            earliest_start = piece_finish_times.get(piece_id, 0.0)
            
            # Very conservative: only attempt parallelization if piece is far from all others
            min_distance_to_others = self._get_min_distance_to_active_pieces(
                piece_id, earliest_start, sequence, optimized_plan, 
                piece_positions_after_moves, initial_positions
            )
            
            # Only try to parallelize if piece is very far from any active pieces (200mm+)
            should_attempt_parallel = min_distance_to_others > 200.0
            
            best_start_time = earliest_start
            
            if should_attempt_parallel and earliest_start < original_start_time:
                # Try to start as early as possible
                collision_free_at_earliest = self._is_sequence_collision_free(
                    piece_id, sequence, earliest_start, optimized_plan, 
                    piece_positions_after_moves, initial_positions
                )
                
                if not collision_free_at_earliest:
                    # Can't start early, use original time
                    best_start_time = original_start_time
            else:
                # Don't attempt parallelization, use original time
                best_start_time = original_start_time
            
            # Add sequence at best time found
            optimized_plan.add_sequence_at_time(sequence, best_start_time)
            
            # Update piece finish time and position after this move
            piece_finish_times[piece_id] = best_start_time + sequence.total_duration
            self._update_simulated_position(piece_positions_after_moves[piece_id], sequence)
            
            # Print if we improved timing
            time_saved = original_start_time - best_start_time
            if time_saved > 0.1:
                parallelized_count += 1
                print(f"  -> {piece_id}: Parallelized, saved {time_saved:.1f}s (starts at {best_start_time:.1f}s)")
        
        new_duration = optimized_plan.get_total_duration()
        original_duration = all_sequences[-1][2] + all_sequences[-1][1].total_duration if all_sequences else 0
        time_improvement = original_duration - new_duration
        
        if time_improvement > 0.1:
            print(f"Parallelized {parallelized_count} moves, total improvement: {time_improvement:.1f}s ({time_improvement/max(original_duration,0.001)*100:.1f}% faster)")
        else:
            print(f"No safe parallelization opportunities found")
        
        return optimized_plan
    
    def _is_sequence_collision_free(self, piece_id: str, sequence: PieceCommandSequence,
                                    start_time: float, plan: ExecutionPlan,
                                    piece_positions: Dict[str, Position],
                                    initial_positions: Dict[str, Position]) -> bool:
        """Check if a sequence would cause collisions with other active sequences"""
        # Sample at very high density for safety (every 0.1s)
        num_samples = max(int(sequence.total_duration * 10), 10)
        
        # Get the starting position for this piece's sequence
        piece_start_pos = piece_positions[piece_id].copy()
        
        for sample in range(num_samples + 1):
            progress = sample / num_samples
            check_time = start_time + sequence.total_duration * progress
            
            # Get this piece's position at this time
            test_pos = self._get_position_at_time(piece_start_pos, sequence, progress)
            
            # Check against all other pieces' positions at this time
            for other_id in plan.sequences.keys():
                if other_id == piece_id:
                    continue
                
                # Find where other piece is at check_time
                other_pos = self._get_piece_position_at_time(
                    other_id, check_time, plan, initial_positions
                )
                
                if other_pos:
                    # Check collision with large safety buffer
                    distance = test_pos.distance_to(other_pos)
                    if distance < PIECE_RADIUS * 2 + 50:  # 50mm safety margin (very conservative)
                        return False
        
        return True
    
    def _get_piece_position_at_time(self, piece_id: str, time: float,
                                    plan: ExecutionPlan,
                                    initial_positions: Dict[str, Position]) -> Optional[Position]:
        """Get a piece's position at a specific time in the plan"""
        if piece_id not in plan.sequences:
            return initial_positions.get(piece_id)
        
        # Start from initial position
        current_pos = initial_positions[piece_id].copy()
        
        # Apply all sequences for this piece up to the given time
        for sequence in plan.sequences[piece_id]:
            seq_start = sequence.start_time
            seq_end = seq_start + sequence.total_duration
            
            if time < seq_start:
                # Time is before this sequence starts
                break
            elif time >= seq_end:
                # Sequence has completed, apply full effect
                self._update_simulated_position(current_pos, sequence)
            else:
                # Time is during this sequence, apply partial progress
                progress = (time - seq_start) / max(sequence.total_duration, 0.001)
                return self._get_position_at_time(current_pos, sequence, progress)
        
        return current_pos
    
    def _get_min_distance_to_active_pieces(self, piece_id: str, start_time: float,
                                           sequence: PieceCommandSequence,
                                           plan: ExecutionPlan,
                                           piece_positions: Dict[str, Position],
                                           initial_positions: Dict[str, Position]) -> float:
        """Get minimum distance from this piece to any active piece during the sequence"""
        min_distance = float('inf')
        
        # Get starting position for this piece
        piece_start_pos = piece_positions[piece_id].copy()
        
        # Sample at a few points during the sequence
        for sample in range(5):
            progress = sample / 4
            check_time = start_time + sequence.total_duration * progress
            test_pos = self._get_position_at_time(piece_start_pos, sequence, progress)
            
            # Check distance to all other pieces at this time
            for other_id in piece_positions.keys():
                if other_id == piece_id:
                    continue
    ...
    def _get_position_at_time(self, start_pos: Position, sequence: PieceCommandSequence,
                             progress: float) -> Position:
        """Calculate piece position at a given progress through a sequence (0.0 to 1.0)"""
        pos = start_pos.copy()
        
        if progress <= 0:
            return pos
        
        # Find which command we're in based on progress
        total_duration = sequence.total_duration
        elapsed = progress * total_duration
        
        for command in sequence.commands:
            if elapsed <= command.duration:
                # We're in this command
                cmd_progress = elapsed / max(command.duration, 0.001)
                
                if command.command_type == CommandType.ROTATE:
                    angle_diff = (command.target_orientation - pos.orientation) % 360
                    if angle_diff > 180:
                        angle_diff -= 360
                    pos.orientation = (pos.orientation + angle_diff * cmd_progress) % 360
                elif command.command_type == CommandType.MOVE_STRAIGHT:
                    angle_rad = math.radians(pos.orientation)
                    distance_traveled = command.distance * cmd_progress
                    pos.x += distance_traveled * math.cos(angle_rad)
                    pos.y += distance_traveled * math.sin(angle_rad)
                
                break
            else:
                # Complete this command and move to next
